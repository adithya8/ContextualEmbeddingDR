...
for i in ${!dbTables[@]}
 do
    ...
    lexTableCr="~/dlatk/dlatk/dlatkInterface.py -d ${db["d"]} -t ${dbTable} -c ${db["c"]} --group_freq_thresh 0 -f 'feat\$bert_ba_un_memimaL10co\$${dbTable}\$${db["c"]}\$16to16' --fit_reducer --model ${dimRedModel} --n_components ${k} --reducer_to_lexicon ${lexTable}${dimRedModel}${k}"
    echo "${lexTableCr}"
    eval "${lexTableCr}"
    echo "----------------------------------------------------------------------"
    weightLex="~/dlatk/dlatk/dlatkInterface.py -d ${db["d"]} -t ${dbTable} -c ${db["c"]} --group_freq_thresh 0 --word_table 'feat\$bert_ba_un_memimaL10co\$${dbTable}\$${db["c"]}\$16to16' --add_lex -l ${lexTable}${dimRedModel}${k} --weighted_lex"
    echo "${weightLex}"
    eval "${weightLex}"
    echo "----------------------------------------------------------------------"
done
...
if [ "$1" -eq "18" ];
then
    eval "cat ~/NLP/ContextualEmbeddingDR/results/clp${1}_adi/BERTb_${dimRedModel}/${resultFile} | grep \'R\':"
else    
    eval "cat ~/NLP/ContextualEmbeddingDR/results/clp${1}_adi/BERTb_${dimRedModel}/${resultFile} | grep \'f1\':"
fi